...
Handles PDF creation and formatting for medical reports
"""
...
def create_paragraphs_from_text(formatted_text, styles):
    ...
    for line in lines:
        ...
        if line.startswith('<b>') and line.endswith('</b>'):
            # Header
            paragraphs.append(Paragraph(line, styles['header']))
        elif line.startswith('    ○'):
            # Sub-bullet
            clean_line = line.replace('    ○ ', '')
            paragraphs.append(Paragraph(f"○ {clean_line}", styles['sub_bullet']))
        elif line.startswith('• '):
            # Main bullet
...
def create_pdf_report(images_and_texts, filename):
    """Create a PDF report with images on left and text on right using A3 size"""
    buffer = BytesIO()
    ...
    for page_num, (image, extracted_text, page_label) in enumerate(images_and_texts, 1):
        ...
        while remaining_paragraphs:
            ...
            if not is_first_page or (page_num > 1 and is_first_page):
                c.showPage()
            ...
            # Draw page footer
            draw_page_footer(c, page_width, margin, page_label, page_num, 
